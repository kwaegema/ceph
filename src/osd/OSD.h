...
public:
  ...
  OSDService(OSDAbstract *osd);
};

class OSDAbstract : public Dispatcher {
  /** OSD **/
public:
...
//  void heartbeat_entry();
  virtual void need_heartbeat_peer_update() = 0;
...
//  friend class C_CompleteSplits;
 ...
 struct OpWQ: public ThreadPool::WorkQueueVal<pair<PGRef, OpRequestRef>,
   ...
   OpWQ(OSDAbstract *o, time_t ti, ThreadPool *tp)
...
//  void enqueue_op(PG *pg, OpRequestRef op);
  virtual void dequeue_op(PGRef pg, OpRequestRef op) = 0;
...
//  // -- peering queue --
 struct PeeringWQ : public ThreadPool::BatchWorkQueue<PG> {
   ...
   PeeringWQ(OSDAbstract *o, time_t ti, ThreadPool *tp, size_t batch_size)
...
//  PGPool _get_pool(int id, OSDMapRef createmap);
  ...
  virtual bool  _have_pg(pg_t pgid) = 0;
//  PG   *_lookup_lock_pg_with_map_lock_held(pg_t pgid);
  virtual PG   *_lookup_lock_pg(pg_t pgid) = 0;
...
//  void flush_pg_stats();
  ...
  virtual void pg_stat_queue_enqueue(PG *pg) = 0;
  virtual void pg_stat_queue_dequeue(PG *pg) = 0;
  virtual void clear_pg_stat_queue() = 0;
...
//  map<pg_t, set<hobject_t> > recovery_oids;
 ...
 struct RecoveryWQ : public ThreadPool::WorkQueue<PG> {
   OSDAbstract *osd;
   RecoveryWQ(OSDAbstract *o, time_t ti, ThreadPool *tp)
 ...
 virtual void start_recovery_op(PG *pg, const hobject_t& soid) = 0;
 virtual void finish_recovery_op(PG *pg, const hobject_t& soid, bool dequeue) = 0;
 virtual void defer_recovery(PG *pg) = 0;
  virtual void do_recovery(PG *pg) = 0;
  virtual bool _recover_now() = 0;
...
//  // -- snap trimming --
 ...
 struct SnapTrimWQ : public ThreadPool::WorkQueue<PG> {
   OSDAbstract *osd;
   SnapTrimWQ(OSDAbstract *o, time_t ti, ThreadPool *tp)
...
//  bool scrub_should_schedule();
 ...
 struct ScrubWQ : public ThreadPool::WorkQueue<PG> {
   OSDAbstract *osd;
   ScrubWQ(OSDAbstract *o, time_t ti, ThreadPool *tp)
 ...
 public:
   ScrubFinalizeWQ(OSDAbstract *o, time_t ti, ThreadPool *tp)
 ...
 public:
   RepScrubWQ(OSDAbstract *o, time_t ti, ThreadPool *tp)
...
protected:
  ...
  OSDAbstract(int id, Messenger *internal_messenger, Messenger *external_messenger, Messenger *hbclientm, MonClient *mc);
  virtual ~OSDAbstract();
...
protected:
  ...
  virtual bool  _have_pg(pg_t pgid);
  PG   *_lookup_lock_pg_with_map_lock_held(pg_t pgid);
  virtual PG   *_lookup_lock_pg(pg_t pgid);
  PG   *_lookup_pg(pg_t pgid);
  PG   *_open_lock_pg(OSDMapRef createmap,
		      pg_t pg, bool no_lockdep_check=false,
		      bool hold_map_lock=false);
