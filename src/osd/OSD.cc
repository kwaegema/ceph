...
void OSD::do_command(Connection *con, tid_t tid, vector<string>& cmd, bufferlist& data)
{
  ...
  if (cmd.empty()) {
	...
	fout << *pg << std::endl;
	std::map<hobject_t, pg_missing_t::item>::const_iterator mend = pg->pg_log.get_missing().missing.end();
	std::map<hobject_t, pg_missing_t::item>::const_iterator mi = pg->pg_log.get_missing().missing.begin();
	for (; mi != mend; ++mi) {
