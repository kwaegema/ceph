 ...
 */
bool PGLog::merge_old_entry(ObjectStore::Transaction& t, pg_log_entry_t& oe, pg_info_t& info, list<hobject_t> &remove_snap_mapped_objects, bool &dirty_log)
{
  ...
  if (log.objects.count(oe.soid)) {
    ...
    dout(20) << "merge_old_entry  had " << oe
	     ...
	     << "deleting" << dendl;
    remove_snap_mapped_objects.push_back(oe.soid);
                      ...
                      bool &dirty_log, bool &dirty_info, bool &dirty_big_info)
{
  ...
  for (list<pg_log_entry_t>::iterator d = divergent.begin(); d != divergent.end(); ++d)
    merge_old_entry(t, *d, info, remove_snap_mapped_objects, dirty_log);
                      ...
                      bool &dirty_log, bool &dirty_info, bool &dirty_big_info)
{
  ...
  if (olog.head < log.head) {
    rewind_divergent_log(t, olog.head, info, remove_snap_mapped_objects, dirty_log, dirty_info, dirty_big_info);
    changed = true;
  }
  ...
  if (olog.head > log.head) {
	...
	if (ne.is_delete())
	  remove_snap_mapped_objects.push_back(ne.soid);
      }
    }
...
void PGLog::_write_log(ObjectStore::Transaction& t, pg_log_t &log,
...
bool PGLog::_read_log(ObjectStore *store, coll_t coll, hobject_t log_oid,
